...
export default function JustCompletedMoviesSection({
  movies = [],
  onPlayMovie,
  onInspectMovie,
}: JustCompletedMoviesSectionProps) {
  const [visibleCount, setVisibleCount] = useState<number>(6);

  const localCompleted = useMemo(() => {
    return movies.filter(m => m.category === 'movie' && m.status === 'Completed');
  }, [movies]);
  ...
  const displayedMovies = useMemo(() => {
    ...
    // Fallback: show any local movies if none are explicitly marked "Completed" yet
    return movies.filter(m => m.category === 'movie').slice(0, 12);
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 transition-all duration-500 ease-in-out border border-purple-500/20 bg-purple-950/5 backdrop-blur-md shadow-[0_0_30px_rgba(168,85,247,0.08)] rounded-3xl mt-6 hover:border-purple-500/45 hover:shadow-[0_0_40px_rgba(168,85,247,0.18)]">
      ...
      <div className="flex items-center justify-between mb-5 border-b border-white/5 pb-3">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-purple-500/20 border border-purple-500/40 flex items-center justify-center text-purple-400 shadow-[0_0_12px_rgba(168,85,247,0.2)]">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Cinematic masterpieces. High-quality theatrical anime releases in one sitting.
            </p>
          </div>
        </div>
      </div>
      ...
      {displayedMovies.length === 0 ? (
        <div className="py-12 bg-white/5 border border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center text-center p-6 gap-2">
          ...
          <h3 className="text-xs font-bold text-gray-300">No Movies Found</h3>
          <p className="text-[11px] text-gray-500 max-w-sm">
            Once you upload anime movies, they will appear here!
          </p>
        </div>
      ) : (
        <>
              ...
              >
                ...
                <div className="relative aspect-[2/3] w-full overflow-hidden bg-zinc-900 border-b border-white/5 shrink-0">
                  ...
                  <div className="absolute top-2 left-2 flex flex-col gap-1 items-start">
                    <span className="px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest bg-gradient-to-r from-purple-500 to-indigo-500 text-white shadow-md">
                      MOVIE
                    </span>
                  </div>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 duration-300 duration-300 gap-2">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        onPlayMovie(movie);
                      }}
                      className="w-10 h-10 rounded-full bg-purple-500 text-white flex items-center justify-center shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 hover:scale-110 active:scale-95"
                    >
                ...
                <div className="p-3 flex flex-col flex-grow justify-between min-w-0">
                  <div className="min-w-0">
                    <span className="text-[10px] font-extrabold uppercase tracking-wider text-purple-400 truncate block">
                      {movie.duration}
                    </span>
                    <h4 className="text-[11px] font-bold text-white mt-1 group-hover:text-purple-300 transition-colors truncate">
                      {movie.title}
                    </h4>
                    {movie.description && (
                  ...
                  <div className="flex items-center justify-between border-t border-white/5 pt-2 mt-2 text-[9px] font-bold uppercase tracking-widest text-gray-500 font-mono">
                    <span className="truncate max-w-[70%]">{movie.genres?.slice(0, 1).join(', ') || 'Movie'}</span>
          ...
          {displayedMovies.length > 6 && (
            <div className="flex justify-center mt-6 pt-2 border-t border-white/5">
              <button
                ...
                className="px-6 py-2.5 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 text-purple-400 border border-purple-500/30 font-bold text-xs uppercase tracking-wider transition-all duration-300 shadow-sm hover:shadow-purple-500/10 flex items-center gap-2 cursor-pointer active:scale-95"
              >
                <span>{visibleCount === 6 ? 'Show More Movies' : 'Show Less'}</span>
                {visibleCount === 6 ? (
